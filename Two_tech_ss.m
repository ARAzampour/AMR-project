function [policy_choice_o,v_new_o,v_new_resh_o,dist_o,trans_matrix_n,...
...
pi_contemp_new      = ((1+a_grid).*(alpha*p_E/p_e)^alpha.*(1/(1+a_grow)).^age_g)...
    .^(1/(1-alpha))*(1-alpha);

cap_contemp_new     = (((1+a_grid).*(alpha*p_E/p_e)^alpha.*(1/(1+a_grow)).^age_g)...
    .^(1/(1-alpha)))';

%%% I also consider the remainder of the firms that have not transitioned

pi_contemp_old      = ((1+a_grid)/tech_dist.*(alpha*p_E/p_e)^alpha.*(1/(1+a_grow)).^age_g)...
    .^(1/(1-alpha))*(1-alpha);

cap_contemp_old     = (((1+a_grid)/tech_dist.*(alpha*p_E/p_e)^alpha.*(1/(1+a_grow)).^age_g)...
    .^(1/(1-alpha)))';
...
for h=1:1:max_iter_measure
    for k=1:1:max_iter_price
        ...
        trans_matrix_o = trans_matrix_o.*(1-exit_vec_n);
        trans_matrix_o = sparse(trans_matrix_o);
        
        dist_n        = m_of_firms_new*ones(1,age_num*a_num_g)/(age_num*a_num_g);
        dist_o        = m_of_firms_new*ones(1,age_num*a_num_g)/(age_num*a_num_g);
        ...
        for j=1:1:max_iter
            dist_new_o    = dist_o *trans_matrix_n;
            dist_new_o    = dist_new_o + (m_of_firms_new-sum(dist_new_o))*dist_ent;
    ...
    value_err_n   = a_prob*(v_new_resh_n(1,:))'-c_of_e;
    ...
    if value_err_n>-5
        m_of_firms_new = m_of_firms_new*(1+0.1*value_err_n);
    else
        m_of_firms_new = m_of_firms_new*(1+0.05*value_err_n);
    end
    if value_err_o>-5
        m_of_firms_old = m_of_firms_old*(1+0.1*value_err_o);
    else
        m_of_firms_old = m_of_firms_old*(1+0.05*value_err_o);
    end
end
end