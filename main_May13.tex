...
\item Their initial technological capital likely plays a role in this: differences between regions
...
\hfill \break
\begin{itemize}
    \item A micro-founded alternative for canon CES models
    \item Analyze the effects of power sector climate policies 
\end{itemize}
\end{center}
\end{frame}
...
\begin{itemize}
    \item We present a heterogeneous agent power sector model
...
\begin{itemize}
 ...
 \item In each period, a generator can choose to pay an adoption cost and update to a new efficiency value (the value drawn is known before paying the cost). It exits if its present value goes to zero
...
\begin{itemize}
 \item Generators use the fuel input $e_i^o$ or $e_i^n$ at prices $p_i^o$ or $p_i^n$, respectively
...
\end{frame}
%%%%%%%%%%%%%%%
\begin{frame}{The model: generator's problem}\label{gen_problem}
\begin{itemize}
...
\end{itemize}
 ...
 \hyperlink{alt_model}{\beamerbutton{Alternative model}}   
  ...
  \hyperlink{rel_eff_why}{\beamerbutton{Relativity}}
...
\begin{frame}{The model: switch from coal to gas}
\begin{itemize}
...
\small 
\begin{itemize}
    \item Since we use an annual time interval, we could also set up a generator problem with a CRS function
    \item The average price that a generator observes would decline as it increases its production
    \item This is due to daily variation in electricity prices and the imperfect substitutability of generators
    \item As a generator increases its production this will occur at times with lower electricity prices $p_E$. Lets say that for the average price that the generator observes we have:
\end{itemize}   
    \small\begin{align*}
        p_E(e_i) = p_0e_i^{-\eta}
    \end{align*}
...
%%%%%%%%%%%%%%%
\begin{frame}{Power plant efficiency distribution}\label{efficall_dist}
...
\hyperlink{Years since latest update}{\beamerbutton{Years since latest update}}

\end{frame}

%%%%%%%%%%%%%%%%

\begin{frame}{Data: Adoption patterns}\label{patterns_1gen}
\footnotesize
\begin{table}
\centering
\caption{Technology adoption dummy regressed on covariates, single-generator plants}
\input{EFFIC_capinc_effic_age_1gen}
\end{table}
...
\includegraphics[scale=0.3]{hist_effic_obscount_1gen.png}
\end{figure}

\hyperlink{patterns_1gen}{\beamerbutton{Efficiency Distribution}}
...
\begin{frame}{Calibration}
    \begin{itemize}
        ...
        \item $\alpha$ will be matched to the aggregate input$/$revenue share: $e^* = \big(\frac{\alpha P_E a_{eff}}{p_e}\big)^\frac{1}{1-\alpha}\rightarrow\frac{p_ee}{p_Ea_{eff}e^\alpha}=\alpha$, or use the price curvature instead?
...
\input{EFFIC_noupd_capageinteract_LOG_1gen}
\end{table}
...
\begin{itemize}
    \item It looks like our model can reproduce the general technology adoption and transition patterns
    \item Need to adjust demand side to get the correct price path pattern
...
%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Model: More Steady state results}\label{moress}
\begin{figure}
\centering
...
%%%%%%%%%%%%%%%%
\begin{frame}{Model: More Steady state results}
\begin{figure}
\centering
...
%%%%%%%%%%%%%%%%
\begin{frame}{Transition 1: Conventional fuels to natural gas}\label{transition1}
...
\includegraphics[scale=0.28]{price path.png}
\end{figure}
...
\centering
\caption{Electricity price over the convergence path}
\includegraphics[scale=0.28]{price_path.png}
\end{figure}
...
\begin{figure}
\caption{The likelihood of tech update and efficiency after previous update}
\includegraphics[scale=0.32]{hist_effic_updatedummy_1gen.png}
\end{figure}
...
\input{EFFIC_capinc_effic_age_all}
\end{table}
\footnotesize{Note: Adoption dummy defined as efficiency increase of at least 5\% with less than 10\% variation in the capacity-to-generation dummy }
...
\centering
\caption{Relative efficiency in periods between updates regressed on covariates}
\input{EFFIC_noupd_capageinteract_LOG_all}
\end{table}
\scriptsize{Note: Relative efficiency denotes efficiency relative to newly updated generators}
...
\includegraphics[scale=0.5]{hist_capage_2020_all.png}
\end{figure}
\scriptsize{Note: Technological update implies an efficiency increase of at least 10 \%, with less than 5 \% variation in generator capacity-to-generation ratio}
...
\footnotesize
\input{summarystats_inpart4_10_5_all_small}
\footnotesize{Note: Adoption dummy defined as efficiency increase of at least 10\% with less than 5\% variation in the capacity-to-generation dummy }
...
\input{summarystats_inpart4_10_5_1gen_small}
...
\end{frame}
%%%%%%%%%%%%%%%
\begin{frame}{Data, part 4: Adoption likelihood and years since latest update}
...
\begin{frame}{Data, part 4: Efficiency and years since latest update}
...
\end{frame}
%%%%%%%%%%%%%%%
\begin{frame}{Results for calibration: Efficiency in periods between updates and years since latest update}
...
%%%%%%%%%%%%%%%
\begin{frame}{Results for calibration: Efficiency in periods between updates and plant average years since latest update}
...
%%%%%%%%%%%%%%%
\begin{frame}{Results for calibration: Efficiency after latest update and plant average years since latest update}
...
\begin{itemize}
\item Where $\alpha<1$ and $\gamma>0$
 \item $\frac{a_{i}^{1+\gamma t}}{(1+g_a)^t}$ is the generator's efficiency relative to the market
 \item $g_a$ is the rate at which a generator's relative efficiency declines
    \item Technology draws follow $a'_i\sim \rho a_i + (1-\rho)\epsilon_i$ where $\epsilon_i$ follows an $i.i.d$ exponential distribution
     \item $\tau$ is the reduction in technological age following an update
