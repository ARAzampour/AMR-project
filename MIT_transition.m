function [trans_prob_o_all,v_new_resh_o_all,dist_o_all,measure_vec_o,...
    trans_prob_n_all,v_new_resh_n_all,dist_n_all,measure_vec_n,...
    age_g,a_grid,a_prob,p_E_vec,cap_old,cap_new] =...
    MIT_transition(a_grow,alpha,~,beta,c_of_a,a_lamb,p_e,a_num_g,age_num,max_iter,...
    v_tol,~,fco,e_p,d_0,c_of_e,dem_tol,init_dist,final_val1,final_val2,...
...
m_of_entry_n        = m_of_entry_n/(sum(m_of_entry_n))*sum(final_dist_n,'all');
m_of_entry_o        = zeros(1,trans_t);
...
%             error       = max(abs(v_of_new-v_new_resh_n),[],"all");
        ...
        for i2=trans_t:-1:1
            ...
            pi_contemp_old      = ((1+a_grid)/tech_dist_vec(i2).*(alpha*p_E_vec(i2)/p_e)^alpha.*(1/(1+a_grow)).^age_g)...
                .^(1/(1-alpha))*(1-alpha);
...
%         m_of_firms_old  = sum(init_dist,'all');
        ...
        for j=1:1:trans_t
            ...
            cap_contemp_new   = (((1+a_grid).*(alpha*p_E_vec(j)/p_e)^alpha.*(1/(1+a_grow)).^age_g)...
                .^(1/(1-alpha)))';
...
%             trans_matrix_n = sparse(trans_matrix_n);
        ...
        for j=1:1:trans_t
            ...
            cap_contemp_old   = (((1+a_grid)/tech_dist_vec(j).*(alpha*p_E_vec(j)/p_e)^alpha.*(1/(1+a_grow)).^age_g)...
                .^(1/(1-alpha)))';
...
%             trans_matrix_o = sparse(trans_matrix_o);
    ...
    if (sum(abs(value_err_n))/trans_t<dem_tol||sum(m_of_entry_n)/trans_t<v_tol) && ...
            (sum(abs(value_err_o))/trans_t<dem_tol||sum(m_of_entry_o)/trans_t<v_tol)
    ...
    if value_err_n>0
        m_of_entry_n = m_of_entry_n.*(1+0.04*value_err_n');
    else
        m_of_entry_n = m_of_entry_n.*(1+0.01*value_err_n');
    end
    if value_err_o>0
        m_of_entry_o = m_of_entry_o.*(1+0.04*value_err_o');
    else
        m_of_entry_o = m_of_entry_o.*(1+0.01*value_err_o');
    end
