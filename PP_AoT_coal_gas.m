...
% for tt = 1:1:100 
    ...
    [trans_prob_old,v_new_old,v_new_resh_old,dist_old,trans_matrix_n_1st,p_e_n_1st,cap_contemp_new,eff_n_final,...
        ...
        Two_tech_ss_AC(a_grow,alpha,a_bar,beta,c_of_a,c_a_new_1st,mu,sigma,a_num_g,age_num,max_iter,...
        v_tol,dist_tol,fco_o,fco_n,e_p,d_0/tech_dist,c_of_e,c_e_new_1st,dem_tol,tech_dist,...
...
[trans_prob_o1,v_new_o1,v_new_resh_o1,dist_o1,trans_matrix_n1,p_e_n1,cap_contemp_n21,eff_n_final21,...
    ...
    Two_tech_ss_AC(a_grow,alpha,a_bar,beta,c_of_a,c_a_new,mu,sigma,a_num_g,age_num,max_iter,...
    v_tol,dist_tol,fco_o,fco_n,e_p,d_0*(1+d0_gr)^trans_t/(tech_dist^(1/(1-alpha))),c_of_e,c_e_new,dem_tol,tech_dist,...
    e0_n_1st,e0_o,e_n_eps,e_o_eps,rho,age_reduc,exo_exit,e_max,gamma);
...
%%%% input is realized
diff_gr_cons    = (1+diff_gr)^(diff_gr_t);
...
eff_n_vec           = (((1+a_grid).*alpha*p_E_vec1(diff_gr_t)/p_e_n_vec1(diff_gr_t).*...
    (((1+a_grid).^gamma)./(1+a_grow)).^age_g).^(1/(1-alpha)))';
eff_n_vec           = min(eff_n_vec,e_max*(1+a_grid)');
eff_o_vec           = (((1+a_grid)/diff_gr_cons.*alpha*p_E_vec1(diff_gr_t)/p_e_o_vec1(diff_gr_t).*...
    (((1+a_grid).^gamma)./(1+a_grow)).^age_g).^(1/(1-alpha)))';
eff_o_vec           = min(eff_o_vec,e_max*(1+a_grid)');
