...
for h=1:1:max_iter_measure
    for k=1:1:max_iter_price
        ...
        eff_m_vec           = (((1+a_grid)/((tech_dist+1)/2).*alpha*p_E/p_e_m.*(1/(1+a_grow)).^age_g)...
            .^(1/(1-alpha)))';
        
        cap_contemp_mix     = (((1+a_grid)/((tech_dist+1)/2).*(alpha*p_E/p_e_m)^alpha.*(1/(1+a_grow)).^age_g)...
            .^(1/(1-alpha)))';
        ...
        for i3=1:1:max_iter
            ...
            v_m_best      = max(v_m_adopt_vec-c_of_a*(1)...
                ,v_p_m_expand);
        ...
        p_e_n       = ((dist_n * eff_n_vec(:)+m_coice*dist_m * eff_m_vec(:))/e0_n).^e_n_eps;
        p_e_o       = ((dist_o * eff_o_vec(:)+(1-m_coice)*dist_m * eff_m_vec(:))/e0_o).^e_o_eps;

        p_e_m       = max(p_e_o,p_e_n);
        m_coice     = p_e_m == p_e_n;
