...
{
  int32_T i;
  p.set_size(&hf_emlrtRTEI, sp, 1, x.size(1));
  i = x.size(1);
  for (int32_T k{0}; k < i; k++) {
    if (k + 1 > x.size(1)) {
      emlrtDynamicBoundsCheckR2012b(k + 1, 1, x.size(1), &ub_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    if ((varargin_1 > 0.0) && (!muDoubleScalarIsNaN(x[k]))) {
      ...
      if (k + 1 > p.size(1)) {
        emlrtDynamicBoundsCheckR2012b(k + 1, 1, p.size(1), &wb_emlrtBCI,
                                      (emlrtCTX)sp);
      }
      p[k] = -b_x;
    } else {
      if (k + 1 > p.size(1)) {
        emlrtDynamicBoundsCheckR2012b(k + 1, 1, p.size(1), &vb_emlrtBCI,
                                      (emlrtCTX)sp);
      }
      p[k] = rtNaN;
    }
