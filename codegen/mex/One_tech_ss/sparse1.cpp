                       ...
                       int32_T ub2)
{
  ...
  b_this->rowidx.set_size(&ce_emlrtRTEI, sp, numAlloc);
  for (overflow = 0; overflow < numAlloc; overflow++) {
    b_this->rowidx[overflow] = 0;
  }
  b_this->d.set_size(&de_emlrtRTEI, sp, numAlloc);
  for (overflow = 0; overflow < numAlloc; overflow++) {
                    ...
                    ::coder::array<real_T, 2U> &c) const
{
  ...
  d_st.tls = c_st.tls;
  if (a.size(1) != m) {
    emlrtErrorWithMessageIdR2018a(sp, &v_emlrtRTEI, "MATLAB:innerdim",
                                  "MATLAB:innerdim", 0);
  }
  st.site = &nf_emlrtRSI;
  c.set_size(&xd_emlrtRTEI, &st, 1, n);
  loop_ub = n;
  ...
  for (int32_T cidx{0}; cidx < sn; cidx++) {
    ...
    if (nzColAlloc < nzRhs) {
      ...
      if (nzColAlloc < extraCol) {
        ...
        rowidxt.set_size(&yd_emlrtRTEI, &c_st, rowidx.size(0));
        k = rowidx.size(0);
        for (i = 0; i < k; i++) {
          rowidxt[i] = rowidx[i];
        }
        dt.set_size(&ae_emlrtRTEI, &c_st, d.size(0));
        k = d.size(0);
        for (i = 0; i < k; i++) {
          dt[i] = d[i];
        }
        ...
        rowidx.set_size(&be_emlrtRTEI, &c_st, nzColAlloc);
        for (i = 0; i < nzColAlloc; i++) {
          rowidx[i] = 0;
        }
        d.set_size(&be_emlrtRTEI, &c_st, nzColAlloc);
        for (i = 0; i < nzColAlloc; i++) {
          d[i] = 0.0;
        }
        maxnz = nzColAlloc;
  ...
  for (tmp = 0; tmp < nidx; tmp++) {
    ...
    if (v1 > partialResults_idx_1) {
      emlrtErrorWithMessageIdR2018a(
          &e_st, &t_emlrtRTEI, "Coder:builtins:IndexOutOfBounds",
